...
**即当空间优化成1维之后，只有完全背包问题的体积是从小到大循环的**

```
从f[i-1][j]转移过来的优化后一般都是从后往后前
```

emmm如果非要把逆序化为正序，可以在第一个for循环中备份一下f数组，用g数组表示，之后更新由g数组更新
...
单调队列优化后(very fast) $O(mn)$：

求滑动窗口内的最大值

```cpp
int n, m;
int f[N], g[N], q[N]; //g表示备份上一次f的数组，q表示一个临时的模拟队列
for(int i=1;i<=n;i++){
    memcpy(g,f,sizeof f);
    //将 f备份到g 
    cin>>v>>w>>s; //输入体积 价值 数量 
    for(int j=0;j<v;j++){ //分拆成v个类 
        int hh=0,tt=-1; //创建一个deque临时队列 
        for(int k=j;k<=m;k+=v){ //对每个类使用单调队列 
            if(hh<=tt&&q[h]<k-s*v) h++; 
            //q[hh] 不在窗口[k-s*v,k-v]内，对头出队 
